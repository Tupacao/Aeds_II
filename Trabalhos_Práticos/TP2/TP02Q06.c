...
void ordenacaoSelecaoRecursiva(Jogador **jogador, int tam, int i) {
    if (i == tam - 1) {
        return;  // Caso base: se já ordenamos até o último elemento, retornamos.
    }
    ...
    if (menor != i) {
        trocar(jogador, i, menor);
    }

    // Chama a função recursivamente para o próximo elemento não ordenado.
    ordenacaoSelecaoRecursiva(jogador, tam, i + 1);
}
...
int main() {
    ...
    ordenacaoSelecaoRecursiva(new, i, 0);
