...
public class TP02Q11 {
    ...
    public static void countingSortByAlturaAndNome(Jogador[] jogadores, int maxAltura) {
        int[] count = new int[maxAltura + 1];
        Jogador[] output = new Jogador[jogadores.length];

        // Conta a frequência das alturas
        for (Jogador jogador : jogadores) {
            count[jogador.getAltura()]++;
        }

        // Atualiza o array de contagem com as posições corretas
        ...
        for (int i = jogadores.length - 1; i >= 0; i--) {
            ...
            output[count[altura] - 1] = jogadores[i];
            count[altura]--;
        }

        // Ordena por nome quando as alturas são iguais
        for (int i = 0; i < maxAltura; i++) {
            int start = count[i];
            int end = (i == maxAltura - 1) ? jogadores.length - 1 : count[i + 1] - 1;
            if (start < end) {
                Arrays.sort(output, start, end + 1, (a, b) -> a.getNome().compareTo(b.getNome()));
            }
        }

        // Copia o array ordenado de volta para o array original
        System.arraycopy(output, 0, jogadores, 0, jogadores.length);
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        countingSortByAlturaAndNome(resp, 190); // Função resposável pela ordenação
